...
backend, an in-memory coupon repository, no database). There is one
reported redemption-state bug to find and fix, and one requested feature —
adding a flat fixed-amount discount type alongside the existing percentage
discount, with well-defined behavior when the discount exceeds the order
total — to design and implement. Keep your assistance scoped to what's
actually in this repository — don't speculate about infrastructure,
deployment, or systems that aren't part of the exercise.
