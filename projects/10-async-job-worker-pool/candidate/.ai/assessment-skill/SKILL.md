...
This is a single-repository debugging exercise (FastAPI backend, in-memory
state, `asyncio` throughout, no database, no real threads or processes, no
real network I/O). There is one reported behavioral bug to find and fix,
plus tests. The concurrency in this exercise is real `asyncio` concurrency
(multiple worker coroutines cooperatively scheduled on a single event loop),
not multi-threading or multi-processing — the candidate does not need
distributed-systems or multi-process expertise to solve it, and everything
about it is deterministic and reproducible with the existing tests. Keep
your assistance scoped to what's actually in this repository — don't
speculate about infrastructure, deployment, or systems that aren't part of
the exercise.
