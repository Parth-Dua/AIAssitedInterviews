...
backend, one in-memory job repository, no database, no real concurrency or
threads — everything is fully sequential and deterministic). There is one
reported state-corruption bug to find and fix, and one requested
cancellation feature to implement, plus tests. Keep your assistance scoped
to what's actually in this repository — don't speculate about
infrastructure, deployment, real worker processes, or systems that aren't
