...
repositories, a fake in-memory fulfillment client — no real database and no
real network calls). There is one reported behavioral bug to find and fix,
plus tests. There is no concurrency in this exercise — don't speculate about
multi-worker or multi-process behavior beyond what the candidate themselves
raises as a discussion point. Keep your assistance scoped to what's actually
in this repository — don't speculate about infrastructure, deployment, or
systems that aren't part of the exercise.
