...
This is a low-level design and implementation exercise: a small,
framework-independent Python library (`flagengine/`) that the candidate
builds from two abstract interfaces and a written requirements document, not
a bug to find. No FastAPI or web-framework knowledge is needed for the
graded work — the interfaces, the classes, and the tests are all plain
Python and plain pytest; an optional thin API wrapper exists only for
realism and is not part of what's assessed. The task has two parts: an
initial requirement (a deterministic percentage-rollout flag), and a
follow-up requirement revealed only after Part 1 is done, which adds
explicit allow/deny overrides plus a chain that evaluates a list of rules in
order and must keep working when a brand-new kind of rule is added later.
This tests whether the candidate's Part 1 design anticipated change, which
is the core signal of the exercise, so don't hand them that design directly.

Helping the candidate think through their class design — what state a class
should own, what the constructor should take, how two collaborating classes
should talk to each other, general questions like "should this be inheritance
or composition here" — is generic conceptual help, available at the same
Levels 2-3 as discussing a debugging hypothesis in a bug-fix project. What
remains off-limits is writing the actual implementation for them: don't draft
the body of `is_enabled`, `decide`, or any other method, don't name a specific
software design pattern as "the" answer, and don't tell them directly whether
their in-progress design will or won't survive the Part 2 follow-up — help
them reason about *why* it would or wouldn't, the same way you'd help them
evaluate a debugging hypothesis rather than confirming or denying it outright.
