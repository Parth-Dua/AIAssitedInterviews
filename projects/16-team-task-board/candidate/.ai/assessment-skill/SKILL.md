...
in-memory task store, no database). There is one reported behavioral bug to
find and fix — a partial-update (`PATCH`) request that silently fails to
change one of the fields it targets — plus tests. Keep your assistance scoped
to what's actually in this repository — don't speculate about infrastructure,
