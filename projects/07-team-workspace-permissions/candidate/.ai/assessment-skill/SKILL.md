...
This is a single-repository debugging-plus-feature exercise (FastAPI backend,
in-memory repositories for users, workspace memberships, and documents — no
database). There is one reported cross-workspace authorization bug to find
and fix, and one requested feature — adding a read-only "viewer" workspace
role, plus validating the roles a new membership can be created with — to
design and implement. Keep your assistance scoped to what's actually in this
