...
export function approveExpenseReport(req: Request, res: Response): void {
  ...
  try {
    ...
    if (err instanceof UnauthorizedApproverError) {
...
export function delegateApproval(req: Request, res: Response): void {
  const { id } = req.params;
  try {
    const report = expenseReportService.delegateApproval(id, req.user!.id, req.body ?? {});
    res.status(200).json(report);
  } catch (err) {
    if (err instanceof ExpenseReportNotFoundError) {
      res.status(404).json({ error: err.message });
