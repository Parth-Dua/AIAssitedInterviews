...
TypeScript backend, in-memory expense-report and user stores, no database,
no real authentication system — a request header stands in for identity).
There is one reported authorization bug to find and fix, and one requested
delegation feature to implement, plus tests. Keep your assistance scoped to
