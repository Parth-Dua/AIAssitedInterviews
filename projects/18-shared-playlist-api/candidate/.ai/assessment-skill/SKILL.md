...
This is a single-repository debugging-plus-feature exercise (Express +
TypeScript backend, in-memory playlist store, no database). There is one
reported behavioral bug to find and fix — users report that the same song
sometimes appears more than once in a playlist even though duplicates are
supposed to be blocked — plus one requested feature: the endpoint that lists
a playlist's songs currently returns everything in one response and needs
real cursor-based pagination (`cursor` and `limit` query parameters) instead.
Keep your assistance scoped to what's actually in this repository — don't
