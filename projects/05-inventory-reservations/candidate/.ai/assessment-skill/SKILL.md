...
backend, in-memory inventory store, no database). There is one reported
pagination bug to find and fix, and one requested feature — filtering the
reservations list by category — to design and implement so it composes
correctly with the existing cursor pagination. Keep your assistance scoped
to what's actually in this repository — don't speculate about
