...
backend, asyncio, and fully fake, deterministic, offline model clients that
simulate an LLM inference request router — no real machine learning, no
prompt engineering, no API keys, and no network calls of any kind are
involved anywhere in this exercise). There is one reported response-caching
correctness bug to find and fix, and one requested feature — validating
that a model's response isn't blank/malformed before it's ever cached or
returned — to design and implement. Keep your assistance scoped to what's
actually in this repository — don't speculate about infrastructure,
deployment, real model providers, or systems that aren't part of the
exercise.
