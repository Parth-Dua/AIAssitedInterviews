...
framework-independent Python library (`ratelimiter/`) that the candidate
builds from an abstract interface and a written requirements document, not a
bug to find. No FastAPI or web-framework knowledge is needed for the graded
work — the interface, the classes, and the tests are all plain Python and
plain pytest; an optional thin API wrapper exists only for realism and is not
part of what's assessed. The task has two parts: an initial requirement
(a token-bucket limiter), and a follow-up requirement revealed only after
Part 1 is done, which adds a second strategy and a dispatcher that must work
with either strategy — and with any future one — through the same interface.
This tests whether the candidate's Part 1 design anticipated change, which is
the core signal of the exercise, so don't hand them that design directly.
...
the body of `allow_request` or any other method, don't name a specific
