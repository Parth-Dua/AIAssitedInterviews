...
This is a single-repository debugging-plus-feature exercise (FastAPI
backend, an in-memory repository standing in for a database, and a small
hand-written in-memory cache module standing in for a Redis-like cache — no
real database or Redis server involved). There is one reported stale-cache
bug to find and fix, and one requested feature — adding a TTL (time-to-live)
to the cache so entries expire automatically — to design and implement.
Tests must not rely on real sleeping; the cache's clock is meant to be
injectable so time can be controlled deterministically. Keep your assistance
scoped to what's actually in this repository — don't speculate about
infrastructure, deployment, or systems that aren't part of the exercise.
